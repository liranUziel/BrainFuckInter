...
int main(int argc, char* argv[]) {
	FILE* fp;
	char* filename;
	char tape[30000] = { 0 }; // the code use a tape as memoey
	char* ptr = tape;
	if (argc < 2) {
	...
	if (fp)
	{
		...
		if (syntaxCheaker(fp) != 0) {
			fprintf(stderr, "Sytax Error mising ] or [");
			return 1;
		}
		//execute code 
...
int execute(FILE* code, int startPoint, char* tape, char* startPtr) {
	...
	while ((cmd = fgetc(code)) != EOF)
	{
		...
		case LEFT:
			if ((--ptr - tape) < 0) {
				fprintf(stderr, "if ((--ptr - tape) < 0)\n");
				return -1;
			}
			break;
		case RIGHT:
			if ((++ptr - tape) > 30000) {
				fprintf(stderr, "if ((++ptr - tape) > 30000)\n");
				return -1;	
			}
			break;
		case LOOP_START:
